  ...
  return new Promise((resolve, reject) => {
    ...
    const readStdout = () => {
      if (!child.stdout || typeof child.stdout === 'number') {
        return Promise.reject(new Error('Claude Code 标准输出不可用'))
      }
      return readLines(child.stdout, handleLine)
    }
    const readStderr = () => {
      if (!child.stderr || typeof child.stderr === 'number') {
        return Promise.reject(new Error('Claude Code 标准错误不可用'))
      }
      return readStream(child.stderr, (text) => {
        stderr += text
      })
    }

    void Promise.all([readStdout(), readStderr(), child.exited])
      .then(([, , code]) => {
        if (settled) return
        if (code !== 0) {
...
function runCodexCompact(
  ...
  return new Promise((resolve, reject) => {
    ...
    const send = (message: Record<string, unknown>) => {
      if (!child.stdin || typeof child.stdin === 'number') {
        fail(new Error('Codex app-server 标准输入不可用'))
        return
      }
    ...
    const readStdout = () => {
      if (!child.stdout || typeof child.stdout === 'number') {
        return Promise.reject(new Error('Codex app-server 标准输出不可用'))
      }
      return readLines(child.stdout, handleLine)
    }
    const readStderr = () => {
      if (!child.stderr || typeof child.stderr === 'number') {
        return Promise.reject(new Error('Codex app-server 标准错误不可用'))
      }
      return readStream(child.stderr, (text) => {
        stderr += text
      })
